...
class ContentTypeBreadcrumbsBuilder implements BreadcrumbBuilderInterface
{
    ...
    public function applies(RouteMatchInterface $attributes)
    {
        try {
            ...
            if ($type === "node" && method_exists($parameters[$type], 'getType')) {
                ...
                if (array_key_exists('type_' . $content_type, $menu_for_type)
                    && $menu_for_type['type_' . $content_type] != ''
                ) {
                    $this->menu_id = str_replace(
                        "main:",
                        '',
                        $menu_for_type['type_' . $content_type]
                    );
            ...
            if ($attributes->getRouteName() == 'entity.taxonomy_term.canonical'
                ...
                if (array_key_exists('vocabulary_' . $vocabulary, $menu_for_type)
                    && $menu_for_type['vocabulary_' . $vocabulary] != ''
                ) {
                    $this->menu_id = str_replace(
                        "main:",
                        '',
    ...
    public function build(RouteMatchInterface $route_match)
    {
        ...
        try {
            ...
            if (count(explode(':', $this->menu_id)) === 1) {
                return $breadcrumb;
            }
            ...
            do {
                ...
                if ($parent_menu) {
                    $menu_tree[$parent_menu->getTitle()] = $parent_menu
                      ->getUrlObject();
            ...
            // if experience any errors in creating breadcrumb
            // then default to homepage and report error to watchdog
            $breadcrumb = new Breadcrumb();
