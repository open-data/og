...
class ConfigForm extends ConfigFormBase
{
    ...
    public function buildForm(array $form, FormStateInterface $form_state)
    {
        try {
            ...
            // Menus
            // @todo support language handling when separate
            // menus are used for each language
            ...
            foreach ($contentTypes as $contentType) {
                ...
                $form['node_group']['type_' . $id] = \Drupal::service('menu.parent_form_selector')
                    ->parentSelectElement(
                        $parent, $config
                        ->get('content_type_breadcrumb.type_' . $id), $menus
                    );
                $form['node_group']['type_' . $id]['#title'] = $this->t($contentType->label());
                $form['node_group']['type_' . $id]['#description'] = $this
                  ->t('Select the parent menu for content type');
                ...
                if ($view) {
                    ...
                    foreach ($displays as $display) {
                        if ($display['display_plugin'] == 'page') {
                            ...
                            $form['view_group']['view_' . $id] = \Drupal::service('menu.parent_form_selector')
                                ->parentSelectElement(
                                    $parent, $config
                                    ->get('content_type_breadcrumb.view_' . $id), $menus
                                );
                            $form['view_group']['view_' . $id]['#title'] = isset($display['display_title'])
                              ? $this->t($display['display_title'])
                              : $id;
    ...
    public function submitForm(array &$form, FormStateInterface $form_state)
    {
        ...
        foreach ($contentTypes as $contentType) {
            $id = $contentType->id();
            $config_value = $form_node_group['type_' . $id] == 'main:'
              ? ''
              : $form_node_group['type_' . $id];
        ...
        foreach ($this->views as $view) {
            $id = $view;
            $config_value = $form_view_group['view_' . $id] == 'main:'
              ? ''
              : $form_view_group['view_' . $id];
        ...
        foreach ($vocabularies as $vocabulary) {
            $id = $vocabulary->id();
            $config_value = $form_vocabulary_group['vocabulary_' . $id] == 'main:'
              ? ''
              : $form_vocabulary_group['vocabulary_' . $id];
            $config->set(
                'content_type_breadcrumb.'
                . 'vocabulary_' . $id, $config_value
            );
        }
